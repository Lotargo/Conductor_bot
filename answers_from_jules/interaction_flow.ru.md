...
sequenceDiagram
    ...
    Sentio-->>App: Report: (emotional_state: {"primary_mood": "радость", ...}, complex_states: [])
    App->>Generator: Сгенерируй ответ. Cистемный промпт: "Ты - ассистент. Твое настроение: радостное. Долгосрочные состояния: нет."
...
4.  **Ваше приложение запрашивает эмоциональный отчет.**
    ...
    *   `Sentio Engine` возвращает `Report` (Отчет), содержащий полное текущее состояние. Например: `{"emotional_state": {"primary_mood": "преобладает грусть", "emotions": {...}}, "complex_states": []}`.
...
## Взаимодействие Эмоций: Механизм Доминантности

Для еще большего реализма, эмоции в `Sentio Engine` не существуют в вакууме. Они взаимодействуют друг с другом на основе концепции **полярных противоположностей** из модели Роберта Плутчика (например, Радость ↔️ Грусть).
...
-   Это не позволяет агенту одновременно испытывать две конфликтующие эмоции с высокой интенсивностью, что делает его состояние более стабильным и правдоподобным. Агент не сможет "разозлиться", пока он по-настоящему "боится".

Этот механизм — шаг к симуляции более сложных "чувств", которые возникают из взаимодействия базовых эмоций.

## Анализ Прошлого: Движок Комплексных Состояний ("Чувств")

`Sentio Engine` способен не только отслеживать сиюминутные *эмоции*, но и выявлять долгосрочные, устойчивые *чувства* или *состояния*. Это достигается путем анализа эмоциональной истории агента за длительный период.

### Как это работает:

1.  **"Рецепты" Чувств:** В конфигурационном файле `feelings.json` определены "рецепты" для различных комплексных состояний. Каждый рецепт — это набор условий, которые должны выполняться в течение определенного времени.
...
2.  **Анализ Истории:** Каждый раз при запросе отчета (`/report`), движок анализирует `EmotionalHistory` за период, указанный в "рецепте".

3.  **Строгая Проверка:** Состояние считается активным, только если **все** условия выполнялись на протяжении **всего** периода. Если хотя бы в один момент времени одно из условий было нарушено (например, произошла короткая вспышка радости), "рецепт" не срабатывает.

4.  **Включение в Отчет:** Если движок обнаруживает активное комплексное состояние, его название добавляется в отчет `Report` в поле `complex_states`.
...
-   Это позволяет AI-агенту сохранять последовательность характера на протяжении многих дней и недель, реагируя на мир через призму своего долгосрочного состояния, что является ключевым шагом к созданию по-настоящему "живых" и правдоподобных виртуальных личностей.
